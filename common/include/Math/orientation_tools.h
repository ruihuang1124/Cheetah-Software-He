...
////    eulrate = T * static_cast<typename Derived1::Scalar>(omegaBody);
//
//  return eulrate;
//}
...
Vec3<Derived> omegaBodyToEulrate(const Vec3<Derived> &eul, const Vec3<Derived> &omegaBody)
{
    Vec3<Derived> eulrate;
    Mat3<Derived> T;
    T.setZero();

    float pitch(eul[1]), roll(eul[2]);
    T(0,1) = std::sin(roll);
    T(0,2) = std::cos(roll);
    T(1,1) = std::cos(roll) * std::cos(pitch);
    T(1,2) = -std::sin(roll) * std::cos(pitch);
