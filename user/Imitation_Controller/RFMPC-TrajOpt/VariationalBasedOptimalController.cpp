...
VariationalBasedOptimalController::VariationalBasedOptimalController()
  ...
  B_LQR_.setZero();
  R_LQR_.resize(3 * VBOC_NUM_CONTACT_POINTS, 3 * VBOC_NUM_CONTACT_POINTS);
  R_LQR_.setZero();
  B_QP_.resize(12, 12);
  B_QP_.setZero();
  P_LQR_.resize(12, 12);
...
void VariationalBasedOptimalController::update_B_LQR() {
  ...
  for (int leg = 0; leg < 4; leg++) {
    if (contact_state_(leg)) contact_legs_++;
  }
  ...
  leg_cnt_ = 0;
  for (int leg = 0; leg < 4; leg++) {
    //    if (contact_state_(leg) > threshold_) {  // Compute B_LQR using only legs in contact
    Eigen::VectorXd rd;
    rd.resize(3, 1);
    tempSkewMatrix3_.setIdentity();
...
//    Eigen::VectorXd rd;
  ...
  for (int i = 0; i < 3 * VBOC_NUM_CONTACT_POINTS; i++) R_LQR_(i, i) = R1_QP_(i, i);
}
...
void VariationalBasedOptimalController::publishDataLcm() {
  lcm_->publish("CONTROLLER_two_contact_stand_data", &two_contact_stand_data_publish_);
}

void VariationalBasedOptimalController::update_log_variables(double* rpy_des_in, double* rpy_act_in) {
  for (int i = 0; i < 3; i++) {
    two_contact_stand_data_.p_des[i] = x_COM_world_desired_(i);
    two_contact_stand_data_.p_act[i] = x_COM_world_(i);
    two_contact_stand_data_.rpy[i] = rpy_des_in[i];
    two_contact_stand_data_.rpy_act[i] = rpy_act_in[i];
  }

  for (int i = 0; i < 12; i++) {
    two_contact_stand_data_.s[i] = s_LQR_(i);
  }

  two_contact_stand_data_.cost_to_go = cost_to_go_;
}
