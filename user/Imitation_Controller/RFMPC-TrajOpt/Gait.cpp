...
OffsetDurationGait::OffsetDurationGait(int nSegment, Vec4<int> offsets, Vec4<int> durations, const std::string& name)
    : _offsets(offsets.array()), _durations(durations.array()), _nIterations(nSegment) {
  _name = name;
...
Vec4<float> OffsetDurationGait::getContactState() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (progress[i] > _durationsFloat[i]) {// _durationsFloat = duration/nSegment
      progress[i] = 0.;
    } else {
...
// progress = 0 means legs in stance.
