...
template<typename T>
void MPC_MSDDP_Solver<T>::initialize() {
...
template<typename T>
void MPC_MSDDP_Solver<T>::update() {
...
template<typename T>
void MPC_MSDDP_Solver<T>::mpcdata_lcm_handler(const lcm::ReceiveBuffer *rbuf, const std::string &chan,
    ...
    mpc_mutex.unlock();
    std::thread solve_mpc_thread(&MPC_MSDDP_Solver::update, this);
...
*/
template<typename T>
void MPC_MSDDP_Solver<T>::update_foot_placement()
{
    // mpc_mutex.lock();
...
template<typename T>
void MPC_MSDDP_Solver<T>::publish_mpc_cmd()
{
...
template<typename T>
void MPC_MSDDP_Solver<T>::publish_debugfoot()
{
    debug_foot_data.N = 0;
...
template
class MPC_MSDDP_Solver<double>;


template<typename T>
void MPC_MSDDP_Solver<T>::updateRFMPCSolver() {
    mpc_mutex.lock(); // lock mpc to prevent updating while the previous hasn't finished
    mpc_iter++;
...
template<typename T>
void MPC_MSDDP_Solver<T>::rfmpcdata_lcm_handler(const lcm::ReceiveBuffer *rbuf, const std::string &chan,
    ...
    mpc_mutex.unlock();
    std::thread solve_mpc_thread(&MPC_MSDDP_Solver::updateRFMPCSolver, this);
    solve_mpc_thread.detach(); // detach the thread from the main thread. The thread would exit once it completes
}


template<typename T>
void MPC_MSDDP_Solver<T>::publish_rfmpc_cmd() {
    int num_controls = mpc_config.nsteps_between_mpc;
...
template<typename T>
int MPC_MSDDP_Solver<T>::switch_leg_index_from_left_to_right(int vbmpc_leg_index) {// in vbmpc, leg index starts from left while in our robot, leg index starts from right.
...
template<typename T>
void MPC_MSDDP_Solver<T>::rpy_to_rotational_matrix_R(Eigen::Matrix<float, 3, 3> &R, float *rpy_in) {
