#ifndef MPC_MSDDP_SOLVER_H
#define MPC_MSDDP_SOLVER_H
...
template<typename T>
class MPC_MSDDP_Solver
{
public:
    MPC_MSDDP_Solver() : mpc_lcm(getLcmUrl(255)), rfmpc_lcm(getLcmUrl(255))
    {
        use_hkd_ = true;
...
//        string state_fname = imitation_path + "state_post_arcdog.csv";
        ...
        if (use_hkd_){
            mpc_lcm.subscribe("mpc_data", &MPC_MSDDP_Solver::mpcdata_lcm_handler, this);
        } else{
            rfmpc_lcm.subscribe("rfmpc_data", &MPC_MSDDP_Solver::rfmpcdata_lcm_handler, this);
        }
...
public:
    // MPC
    HKDProblemMS<T> opt_problem;
    HKDProblemMSData<T> opt_problem_data;
...
template<typename T>
void MPC_MSDDP_Solver<T>::print_mc_state()
{
    printf("***********mc state*************\n");
