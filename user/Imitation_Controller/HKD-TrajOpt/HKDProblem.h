...
template<typename T>
struct HKDProblemMSData
{
    HKDReference<T>* reference_ptr;
    HKDReferenceData<T>* ref_data_ptr;

    deque<shared_ptr<Trajectory<T,24,24,0>>> trajectory_ptrs;
    deque<shared_ptr<MSSinglePhase<T,24,24,0>>> phase_ptrs;
};
...
public:
    HKDProblemMS(){}
    HKDProblemMS(HKDProblemMSData<T>* pdata_in, const HKDPlanConfig<T>& config) {
        setup(pdata_in, config);
    }

    void setup(HKDProblemMSData<T>* pdata_in, const HKDPlanConfig<T>& config){
        plan_duration = config.plan_duration;
        timeStep = config.timeStep;
        nsteps_between_mpc = config.nsteps_between_mpc;
        time_between_mpc = timeStep * nsteps_between_mpc;
    ...
    void create_problem_one_phase(shared_ptr<MSSinglePhase<T,24,24,0>> phase, int idx);
