...
template <typename T>
void HKDProblemMS<T>::initialization()
{
    deque<int> &horizons = ref_data->horizons;  // the ref_date is the most important parameters we need.!!
    deque<VecM<int, 4>> &ctactSeq = ref_data->contactSeq;
    int n_phases = ref_data->n_phases; // see MPCSolver first set data in HKDProblem. the ref_data n_phases is set
    ...
    for (int i = 0; i < n_phases; i++)
    {
        shared_ptr<MSSinglePhase<T,24,24,0>> phase;
        phase = make_shared<MSSinglePhase<T,24,24,0>>();
...
//            traj_to_add->update_nominal_control_vals(ref_data->n_phases - 1); // only the last phase of ref_data.
            ...
            shared_ptr<MSSinglePhase<T, 24, 24, 0>> phase_to_add;
            phase_to_add = make_shared<MSSinglePhase<T, 24, 24, 0>>();
...
template<typename T>
void HKDProblemMS<T>::create_problem_one_phase(shared_ptr<MSSinglePhase<T,24,24,0>> phase, int idx)
...
template<typename T>
void HKDProblemMS<T>::lcm_publish()
{
    int k(0);
    float timeStart = ref_data->startTimes[0];
...
template<typename T>
void HKDProblemMS<T>::reset_lcm_data()
{
    lcm_pdata.times.clear();
    lcm_pdata.contacts.clear();
    lcm_pdata.pos_r.clear();
...
template class HKDProblemMS<double>;