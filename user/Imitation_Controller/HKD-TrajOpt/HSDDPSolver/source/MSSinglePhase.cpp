...
MSSinglePhase<T,xs,us,ys>::MSSinglePhase()
{
    Qx.setZero();
    Qu.setZero();
    Qxx.setZero();
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::initialization()
{
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::set_initial_condition(DVec<T> &x0_)
{
    X->at(0) = x0_;
}

template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::set_nominal_initial_condition(DVec<T> &x0_)
{
    Xbar->at(0) = x0_;
    dbar->at(0).setZero();
}
template <typename T, size_t xs, size_t us, size_t ys>
DVec<T> MSSinglePhase<T,xs,us,ys>::resetmap(DVec<T> &x_)
{
    DVec<T> xnext;
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::resetmap_partial(DMat<T>& Px_, DVec<T> &x_)
{
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::get_value_info_at_init(T &dV_, DVec<T> &G_, DMat<T> &H_)
{
    dV_ = dV->at(0);
    G_ = G->at(0);
    H_ = H->at(0);
...
template <typename T, size_t xs, size_t us, size_t ys>
DVec<T> MSSinglePhase<T,xs,us,ys>::get_terminal_state()
{
    return X->back();
}

template <typename T, size_t xs, size_t us, size_t ys>
DVec<T> MSSinglePhase<T,xs,us,ys>::get_terminal_state_nominal()
{
    return Xbar->back();
}

template <typename T, size_t xs, size_t us, size_t ys>
T MSSinglePhase<T,xs,us,ys>::get_actual_cost()
{
...
template <typename T, size_t xs, size_t us, size_t ys>
T MSSinglePhase<T,xs,us,ys>::get_max_tconstrs()
{
    return constraintContainer.get_max_tconstrs();
}

template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::set_trajectory(shared_ptr<Trajectory<T,xs,us,ys>> traj_)
{
    traj = traj_;
...
void MSSinglePhase<T,xs,us,ys>::forward_sweep(T eps, HSDDP_OPTION &option, bool calc_partial)
{
    ...
    for (k = 0; k < phase_horizon; k++)
    {
        ...
        /* run hybrid roll-out */
        // if
        if ((k+1)/steps_between_shooting_states == 0){ // X(k+1) is a shooting state
            X->at(k + 1) = Xbar->at(k + 1) + A->at(k) * (X->at(k) - Xbar->at(k)) +
                           B->at(k) * (eps * dU->at(k) + K->at(k) * (X->at(k) - Xbar->at(k))) + eps * dbar->at(k + 1);
            VecM<T, xs> dynamic_X;
            dynamic_X.setZero();
...
template <typename T, size_t xs, size_t us, size_t ys>
bool MSSinglePhase<T,xs,us,ys>::backward_sweep(T regularization, T dVprime, DVec<T> Gprime, DMat<T> Hprime)
{
    bool success = true;
    ...
    for (int k = phase_horizon - 1; k >= 0; k--)
    {
        ...
        auto &Ck = C->at(k);
        auto &Dk = D->at(k);
        auto &dk_bar_next = dbar->at(k+1);
        auto &Gnext = G->at(k + 1);
        auto &Hnext = H->at(k + 1);

        Qx = rcost.lx + Ak.transpose() * (Gnext + dk_bar_next) + Ck.transpose() * rcost.ly;
        Qu = rcost.lu + Bk.transpose() * (Gnext + dk_bar_next) + Dk.transpose() * rcost.ly;
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::update_running_cost_with_pconstr(RCostData<T, xs, us, ys> &rcost,
                                                               vector<IneqConstrData<T, xs, us, ys>> &pconstrsData,
                                                               vector<REB_Param_Struct<T>> &reb_params,
                                                               bool flag)
{
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::update_running_cost_with_smooth()
{
}

template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::update_terminal_cost_with_tconstr(vector<TConstrData<T, xs>> &tconstrsData,
                                                                vector<AL_Param_Struct<T>> &al_params,
                                                                bool flag)
{
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::update_nominal_trajectory()
{
    traj->update_nominal_vals();
}

template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::compute_barrier(vector<IneqConstrData<T, xs, us, ys>> &pconstrsData,
                                              vector<REB_Param_Struct<T>> &reb_params)
{
    Bar.setZero();
    Bard.setZero();
...
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::update_trajectory_ptrs()
{
    if (traj == nullptr)
    {
    ...
    steps_between_shooting_states = traj->steps_between_shooting_states;
}

template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::update_AL_params(HSDDP_OPTION& option)
{
    constraintContainer.update_al_params(option);
}
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::update_REB_params(HSDDP_OPTION& option)
{
...
*/
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::push_back()
{
    traj->push_back_zero();
...
*/
template <typename T, size_t xs, size_t us, size_t ys>
void MSSinglePhase<T,xs,us,ys>::pop_front()
{
    traj->pop_front();
...
template class MSSinglePhase<double, 24, 24, 0>;