    ...
    while (getline(fstrm, line))
    {
        ...
        while (getline(s, word, ','))
        {
            if (i == 0){
                ...
                x[i - 1] = static_cast<T>(std::stod(word));
            }
            i++;
        }
        if (idx == pidx)
...
template <typename T>
void Imitation_Reference<T>::load_state_data_with_control(const string &state_fname) {
    auto &Xr = data.Xr;// top level reference data.
    auto &Ur = data.Ur;
    auto &Yr = data.Yr;
    auto &horizons = data.horizons;
    ...
    while (getline(fstrm, line))
    {
        ...
        while (getline(s, word, ','))
        {
            if (i == 0){
                ...
                u[i - 25] = static_cast<T>(std::stod(word));
            }
            i++;
        }
        if (idx == pidx)
...
//        Ur.push_back(Ur_phase);
        Yr.push_back(Yr_phase);
        if(Xr[i].size() == Ur[i].size()){
            printf(RED);
            printf("Ur[i].size() is %d\n",Ur[i].size());
            printf("Yr[i].size() is %d\n",Yr[i].size());
            printf(RESET);
        }
    }
    printf("finished loading imimitation reference \n \n");
