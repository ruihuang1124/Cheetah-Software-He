...
#include "FollowTrajectoryCtrl_Utils.h"
#include "cTypes.h"
#include "utilities.h"
#include <unistd.h>
#include <chrono>
...
Follow_Trajectory_Controller::Follow_Trajectory_Controller() : mpc_cmds_lcm(getLcmUrl(255)), mpc_data_lcm(getLcmUrl(255)), rfmpc_data_lcm(getLcmUrl(255))
{
    ...
    in_standup = false;
    finish_trajectory_following = false;
...
void Follow_Trajectory_Controller::initializeController()
{
    mpc_cmds_lcm.subscribe("mpc_command", &Follow_Trajectory_Controller::handleMPCcommand, this);
    mpcLCMthread = std::thread(&Follow_Trajectory_Controller::handleMPCLCMthread, this);
    iter = 0;
    mpc_time = 0;
    iter_loco = 0;
...
void Follow_Trajectory_Controller::handleMPCLCMthread()
{
    while (true)
    {
...
void Follow_Trajectory_Controller::handleMPCcommand(const lcm::ReceiveBuffer *rbuf, const std::string &chan,
...
void Follow_Trajectory_Controller::runController()
{
    iter++;

    if (!finish_trajectory_following){
        ...
        desired_command_mode = RC_MODE::rc_standup;
    }
    ...
    yaw = raw_yaw_cur + 2*3.1415926*yaw_flip_plus_times - 2*3.1415926*yaw_flip_mins_times;
...
void Follow_Trajectory_Controller::locomotion_ctrl()
{    
    getContactStatus();
    getStatusDuration();
...
void Follow_Trajectory_Controller::update_mpc_if_needed()
{
...
*/
void Follow_Trajectory_Controller::get_a_val_from_solution_bag()
{
    mpc_cmd_mutex.lock();
    for (int i = 0; i < mpc_cmds.N_mpcsteps - 1; i++)
    {
        if (mpc_time > mpc_cmds.mpc_times[i] || almostEqual_number(mpc_time, mpc_cmds.mpc_times[i]))
        {
            if (mpc_time < mpc_cmds.mpc_times[i + 1])
            {
                ...
                if (i>=10) {
                    in_standup = false; // set initial pos as current for stand up ctrl.
                    finish_trajectory_following = true;
                } // the trajectory is finished!, back to stand up.
...
void Follow_Trajectory_Controller::draw_swing()
{
    for (int foot = 0; foot < 4; foot++)
    {
...
void Follow_Trajectory_Controller::standup_ctrl()
{
    if (!in_standup)
    {
        standup_ctrl_enter();
    ...
    Vec4<float> currentStanceState(0.5,0.5,0.5,0.5);
    _stateEstimator->setContactPhase(currentStanceState);
}

void Follow_Trajectory_Controller::standup_ctrl_enter()
{
    iter_standup = 0;
...
void Follow_Trajectory_Controller::standup_ctrl_run()
{
...
void Follow_Trajectory_Controller::getContactStatus()
{
    mpc_cmd_mutex.lock();
    for (int i = 0; i < mpc_cmds.N_mpcsteps - 1; i++)
    {
...
void Follow_Trajectory_Controller::getStatusDuration()
{
    mpc_cmd_mutex.lock();
    for (int i = 0; i < mpc_cmds.N_mpcsteps - 1; i++)
    {
...
void Follow_Trajectory_Controller::avoid_leg_collision_CT(int i)
{
...
void Follow_Trajectory_Controller::compute_knee_position(Vec3<float>&p, Vec3<float>&q, int leg)
{
